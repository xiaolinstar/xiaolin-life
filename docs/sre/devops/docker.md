...
> 容器镜像配置文件为 `Dockerfile`；打包指令为 `docker build` 或 `docker buildx build`

基于源代码，构建静态资源包 *dist* 和 使用 Nginx 静态资源代理生成容器镜像，2个动作在同一个 `Dockerfile` 中实现。
...
- 容器化：将构建产物与其运行环境（依赖、配置、运行时）一起封装为一个轻量级、可移植的容器镜像
